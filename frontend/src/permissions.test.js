...
describe("MEMBER — edit access, no admin actions", () => {
  ...
  it("can change_issue_status if assignee", () => expect(can(MEMBER, ACTIONS.CHANGE_ISSUE_STATUS, { isAssignee: true })).toBe(true));
  it("cannot change_issue_status if not assignee", () => expect(can(MEMBER, ACTIONS.CHANGE_ISSUE_STATUS, { isAssignee: false })).toBe(false));
  it("cannot change_issue_status with no context", () => expect(can(MEMBER, ACTIONS.CHANGE_ISSUE_STATUS)).toBe(false));
